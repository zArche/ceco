...
def save_cache(tag, cache):
    with open(tag + CACHE_FILE_NAME, "wb") as fout, open(tag + CACHE_FLAG_FILE_NAME, "wb") as fout2:
        fout.write(cache)
        fout2.write(str(get_timestamp_of_now()))


def get_cache(tag):
    with open(tag + CACHE_FILE_NAME, "rb") as fin:
        cache = fin.read()
    return cache
...
def is_cache_invalid(tag):
    try:
        with open(tag + CACHE_FLAG_FILE_NAME, "rb") as fin:
